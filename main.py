...
# os.environ["CUDA_VISIBLE_DEVICES"] = "2,3"#没法取
...
init_phases = [20,20,20,20]         #初始相位时间
...
mainQN = Qnetwork(h_size,np.int32(action_num),1)      # 控制we绿灯的网络
targetQN = Qnetwork(h_size,np.int32(action_num),2)

mainQN2 = Qnetwork(h_size,np.int32(action_num),3)    # 控制ns绿灯的网络
targetQN2 = Qnetwork(h_size,np.int32(action_num),4)
...
# 初始化经验池,此处可以改成experience_buffer()则为无优先级排序的记忆池
myBuffer0 = priorized_experience_buffer()         # we buffer
myBuffer1 = priorized_experience_buffer()         # ns buffer
...
ck_wfile = open(path+'acwait-ck.txt', 'w')
ck_avwfile = open(path+'avwait-ck.txt', 'w')
ck_impfile = open(path+'impatience-ck.txt', 'w')

rfile = open(path+'reward-r2.txt', 'w')
wfile = open(path+'wait-r2.txt', 'w')
awfile = open(path+'acc-wait-r2.txt', 'w')
tfile = open(path+'throput-r2.txt', 'w')
r_output = open('output/rewardoutput10_726.csv','w+')
#r_output = open('C:/Users/Administrator/Desktop/trafficSignalControl/CSP/src/main/RL/output/reward.csv','w+')
q_output = open('output/queueoutput10_726.csv','w+')
...
for i in range(1, num_episodes):
    episodeBuffer0 = priorized_experience_buffer()        # we首先明确好经验池的大小问题
    episodeBuffer1 = priorized_experience_buffer()        # ns
    environment = sumo.reset()                                    # 重置环境
    current_state = sumo.getState()                                      # 获取初始环境的状态信息
    current_phases = list(init_phases)                    # 将初始相位设置成list
    wait_time_map = {}
    wait_time = 0                                         # 记录等待时间
    done = False
    rAll = 0                                              # 奖励值进行不断的累加
    j = 0                                                 # 一个episode下回合数累加器
    ...
    pre_throughput = 20
    pre_impatience = 0.01
    ns = 1       # 等于1表示下一个周期是ns绿灯


    print('step:' + str(i) + ',precent:%.3f' % (i / num_episodes * 100) + '%')
    while j < max_epLength:    # 一个eposide的周期数不超过max_eplength
        j+=1
        if(ns):
            ...
            next_state, reward, done, wait_time_map, q_m, current_queue, current_delay, current_throughput, impatience = sumo.step(environment, cycle, wait_time_map, pre_queue, pre_delay, pre_throughput, pre_impatience)
        else:    # 下一个周期是we绿灯
            current_phases = sumo.getCorrectCycle(current_phases)
            legal_action = sumo.getLegalAction(current_phases)
            action = mainQN2.choose_action(epsilon, current_state, legal_action, sess, flag)
            cycle = sumo.getPhaseFromAction(current_phases, action)
            next_state, reward, done, wait_time_map, q_m, current_queue, current_delay, current_throughput, impatience = sumo.step(environment, cycle, wait_time_map, pre_queue, pre_delay, pre_throughput, pre_impatience)
        total_steps += 1
        ...
        legal_act_s1 = sumo.getLegalAction(cycle)#下 一个交通相位中的可选行为
        legal_a_one_s1 = [0 if x!=-1 else -99999 for x in legal_act_s1] #l egal_a_one和legal_a_one_s1分别是前一轮相位配置和当前相位配置对应动作空间的值
        if(ns):
            ...
            episodeBuffer1.add(np.reshape(np.array([current_state, action, reward, next_state, done, legal_a_one, legal_a_one_s1]),[1, 7]))
        if total_steps > pre_train_steps:
            flag = True                                                   # 达到此要求开始训练神经网络
            if(ns):
                if epsilon > endE:
                ...
                if total_steps % (update_freq) == 0:                                             # 多少步来更新一次网络
                    ...
                    myBuffer0.updateErr(indx, err)                                              # 更新记忆池优先级
                ...
                if epsilon > endE:
                    epsilon -= stepDrop  # 选择动作的eplison
                if total_steps % (update_freq) == 0:  # 多少步来更新一次网络
                    trainBatch = myBuffer1.priorized_sample(batch_size)  # Get a random batch of experiences.
                    # indx = np.reshape(np.vstack(trainBatch[:, 1]), [batch_size])  # 取训练集中所有的执行动作a（即第二个值）。
                    # indx = indx.astype(int)  # 修改indx的数据类型
                    trainBatch = np.vstack(trainBatch[:, 0])  # 取训练集中的状态（指当前状态，第一个值）
                    ...
                    # action from the main QN
                    # 当前网络Q值，动作
                    Q1 = sess.run(mainQN2.predict, feed_dict={mainQN.scalarInput: np.vstack(trainBatch[:, 3]),
                                                             mainQN.legal_actions: np.vstack(trainBatch[:, 5])})
                    # 目标网络Q值
                    Q2 = sess.run(targetQN2.Qout, feed_dict={targetQN.scalarInput: np.vstack(trainBatch[:, 3]),
        ...
        avwep += (current_delay )/len(wait_time_map)

        pre_delay = current_delay
        pre_throughput = current_throughput
        pre_impatience = impatience

        accwait += current_delay
        acimp += impatience
    ...
    print("episode waittime total",accwait)
    print("average wait time", avwep)
    print("acimp",acimp)
    ck_wfile.write(str(accwait)+'\n')
    ck_avwfile.write(str(avwep)+'\n')
    ck_impfile.write(str(acimp)+'\n')
...
saver.save(sess,path+'/model-'+str(i)+'.cptk')
saver.save(sess,path+'/model-'+'ck_6_10'+'.cptk')
